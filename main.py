...
<:icon9:1420479660222841063> Speaking to Director/SHR+, told by HR to open and etc."""
    ...
    # Message visible to everyone, runner hidden
...
class TicketDropdown(ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Ping creator with link (ephemeral)
        ...
        await log_action(
            ...
            details=f"Ticket Type: {self.values[0]}"
        )
...
class UnclaimButton(ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Toggle buttons back: Claim + Close
        view = ui.View()
        view.add_item(ClaimButton())
        view.add_item(CloseButton())
        await ticket_msg.edit(embed=embed, view=view)
...
class CloseButton(ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await log_action(
            ...
            user=interaction.user,
            channel=ticket_channel,
            details=f"Transcript saved: logs/purged_messages_{ticket_channel.id}.txt"
        )
...
async def log_action(action, user, channel, details=None):
    ...
    embed = discord.Embed(
        title=f"<:emoji_1:1401614346316021813> {action}",
...
# --- Add the ticket dropdown to a view for /panel ---
class TicketDropdown(ui.Select):
    def __init__(self):
        options = [
            discord.SelectOption(label="Desk Support", description="For general issues/questions"),
